	...
	private void spotCheck(int checkRow, int checkCol, int boardRow, int boardCol, int connectionA) {
		...
		if (isOpen(checkRow, checkCol)) {
			...
			if (isFull(checkRow, checkCol)) {
				 ...
				 * 
				 * unionObj.union(board[row - 1][col],board[row][col]) board[row][col] =
				 * board[row - 1][col]
				 */
		 ...
		 * 
		 * (row == (this.side - 1) && col == (this.side - 1)) (row == (this.side - 1) &&
		 * col == 1)
		 */
...
//    			this.unionObject.find(gridToNumber(row, col)) == (weightedQuickUnionLength - 2);
	...
	public boolean percolates() {
		...
		// Potential Change
		// return (this.unionObject.find(weightedQuickUnionLength - 1) ==
		// this.unionObject.find(weightedQuickUnionLength - 2));
